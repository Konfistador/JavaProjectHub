...
public interface Range<R extends Range<R, P, D>, P extends Comparable<? super P>, D extends Comparable<? super D>>
    ...
    default boolean contains( P point ) {
        if (start().equals(point)) return true;
        return min(start(), point).equals(start()) && max(end(), point).equals(end()) && !end().equals(point);
    }
    ...
    default boolean overlaps( R other ) {
        P b = max(this.start(), other.start());
        P c = min(this.end(), other.end());
...
//    long length(Object unit);
    ...
    default D overlap( R other ) {
        if (!this.overlaps(other)) return zero();
       // if(this.end().compareTo(other.start()) <= 0) return this.zero();

        return between(min(this.end(), other.end()), max(this.start(), other.start())).length();
    }
    ...
    default R joinWith( R other ) throws IllegalArgumentException {
        if(this.compareTo(other) < 0) {
          ...
          return other.joinWith(this.self());
        }
    }
    ...
    default Optional<R> intersectWith( R other ) {
        if (!this.overlaps(other)) return Optional.empty();
         return Optional.of(between(min(this.end(), other.end()), max(this.start(), other.start())));
    }
    ...
    default boolean contains( R other ) {
        if (this.start().equals(other.start()) || min(this.start(), other.start()).equals(this.start())) {
            if (this.end().equals(other.end()) || max(this.end(), other.end()).equals(this.end())) return true;
        }
        return false;
    }
